# running args: run train.py --dataset ShanghaiA  --save_path ./save_file/ShanghaiA batch_size 24 --pre ./save_file/A_baseline/checkpoint_pvtv2.pth
...
from config_pvtv2_ft_1to4_6912x4_shA import return_args, args
import numpy as np
...
def main(args):
    ...
    criterion =  nn.SmoothL1Loss(size_average=False,beta=1.0).cuda()    
...
def train(Pre_data, model, criterion, optimizer, epoch, args, scheduler):
    ...
    for i, (fname, img, gt_count) in enumerate(train_loader):
        ...
        out = out.type(torch.FloatTensor).cuda().unsqueeze(1)
         
        gt_count = gt_count.type(torch.FloatTensor).cuda().unsqueeze(1)

#        print('gt shape:',gt_count.shape)
#        print('et shape:',out.shape)

        gt=torch.sum(gt_count)
        et=torch.sum(out)
